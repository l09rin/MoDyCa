...
class soft : public interaction<particle> {     // MD  // energy in units of EPSILON
 ...
 private:
  particle force ;
  double multiplier = 0 , r6_inv = 0 , r2_inv = 0 ;
...
template <typename particle>
void soft<particle>::generate_by_record( record *file_rec , const char *simulation_kind , configuration<particle> *conf_ptr , int parts_number ) {
	              // CUTOFF
...
template <typename particle>
inline double soft<particle>::pair_potential( particle *i_part , particle *j_part , double distance ) {  // MC
  /**********  LENNARD-JONES -- SOFT POTENTIAL  ********/
...
template <typename particle>
inline particle soft<particle>::pair_force( particle *i_part , particle *j_part , double distance ) {
    /***********   LENNARD-JONES - SOFT FORCE     **********/
                                             // forces are in units of EPSILON/SIGMA
  r2_inv = 1.0 / ( distance*distance ) ;
  r6_inv = r2_inv * r2_inv ;
  multiplier = - 24.0 * r6_inv * r6_inv ;
...
template <typename particle>
double soft<particle>::return_pair_potential( double distance ) {
  return pair_potential( NULL , NULL , distance ) + SHIFT ;
};
/********************************************************************************************/

template <typename particle>
double soft<particle>::return_pair_potential( particle *part1 , particle *part2 ) {
  double energy = 0 ;
  double dist = 0 ;
...
template <typename particle>
particle soft<particle>::return_pair_force( particle *i_part , particle *j_part ) {
...
template <typename particle>
double soft<particle>::compute_energy_contribution( void ) {
  double energy = 0 ;
  double dist = 0 ;
...
template <typename particle>
inline double soft<particle>::compute_energy_contribution( particle *part ) {
...
template <typename particle>
double soft<particle>::delta_energy_constrained_CoM( particle *disp ) {
...
template <typename particle>
double soft<particle>::lambda_derivative_perpart( void ) {
...
template <typename particle>
void soft<particle>::compute_forces_contribution(void) {
  double dist = 0 ;
  neighbour_list<particle> *walker = NULL ;
...
template <typename particle>
double soft<particle>::compute_virial_contribution(void) {
  double dist = 0 ;
  neighbour_list<particle> *walker = NULL ;
...
template <typename particle>
string soft<particle>::ostr( void ) {
  ostringstream st ;
  st << "SOFT potential has no further parameters dependence." << endl ;
  return st.str() ;
};
/********************************************************************************************/

template <typename particle>
soft<particle> *soft<particle>::copy( void ) {
  soft *copy = new soft() ;
  return copy ;
};
/********************************************************************************************/
...
template <typename particle>
void soft<particle>::compute_per_part_energy_contribution( double *envec ) {
  double energy = 0 ;
  double dist = 0 ;
  neighbour_list<particle> *walker = NULL ;
  particle *apparent_near = NULL ;
...
template <typename particle>
void soft<particle>::compute_per_part_virial_contribution( double *virvec ) {
  double dist = 0 ;
  neighbour_list<particle> *walker = NULL ;
  particle *apparent_near = NULL , deltaF ;
  apparent_near = new particle ;
...
template <typename particle>
void soft<particle>::build_bonds_network( double bonds_cutoff ) {
  cout << "*** WARNING : no method defined to build bonds_network" << endl ;
};
/********************************************************************************************/

template <typename particle>
double * soft<particle>::dump_parameters( void ) {
  double *params = NULL ;
  return params ;
};
/********************************************************************************************/
