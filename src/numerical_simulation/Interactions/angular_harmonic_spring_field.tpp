/*****************************************************************************************/
/* This is an external field for which each particle's orientation is (periodically) bounded with an harmonic spring to its initial value */

template <typename particle>
class angular_h_spring_field : public interaction<particle> { // energy in units of EPSILON
 ...
 public:
  ...
  angular_h_spring_field( double Lambda = 0.0 , double Alpha_theta = 1.0 , int N_minima = 1 ) ;
 private:
 ...
 public:
  ...
  angular_h_spring_field *copy( void ) ;
  ~angular_h_spring_field() ;
};
...
template <typename particle>
angular_h_spring_field<particle>::angular_h_spring_field( double Lambda , double Alpha_theta , int N_minima ) {
  lambda = Lambda ;
  alpha_theta = Alpha_theta ;
  n_minima = N_minima ;
  HALF_PERIOD = M_PI / n_minima ;
  PERIOD = 2 * HALF_PERIOD ;
  sprintf( name , "%s" , "ang_h_spring\0" ) ;
};
/********************************************************************************************/

template <typename particle>
angular_h_spring_field<particle>::~angular_h_spring_field( void ) {
...
template <typename particle>
void angular_h_spring_field<particle>::generate_by_record( record *file_rec , const char *simulation_kind , configuration<particle> *conf_ptr , int parts_number ) {
  cout << "*** ERROR : ANGULAR-HARMONIC-SPRING potential can only be used with the particle class patchy_2D !!" << endl ;
  exit(EXIT_FAILURE) ;
}

template <>
void angular_h_spring_field<patchy_2D>::generate_by_record( record *file_rec , const char *simulation_kind , configuration<patchy_2D> *conf_ptr , int parts_number ) {
...
template <typename particle>
inline double angular_h_spring_field<particle>::potential( particle *part , double theta0 ) {
  cout << "*** ERROR : ANGULAR-HARMONIC-SPRING potential can only be used with the particle class patchy_2D !!" << endl ;
  exit(EXIT_FAILURE) ;
}

template <>
inline double angular_h_spring_field<patchy_2D>::potential( patchy_2D *part , double theta0 ) {
  /***********   ANGULAR HARMONIC EXTERNAL POTENTIAL     Vi = lambda * alpha_theta * (rotation_bar / HALF_PERIOD)^2 ;    **********/
  /**********                                    rotation_bar = (rotation-theta0+pi/n_minima) % (2*pi/n_minima) - pi/n_minima    ***************/
  static double theta_bar ;
  theta_bar = part->rotation - theta0 ;
  if( theta_bar > HALF_PERIOD ) theta_bar = theta_bar - floor( (theta_bar/HALF_PERIOD+1)/2 ) * PERIOD ;
  else if( theta_bar < -HALF_PERIOD ) theta_bar = theta_bar - ceil( (theta_bar/HALF_PERIOD-1)/2 ) * PERIOD ;
  theta_bar /= HALF_PERIOD ;
  return lambda * alpha_theta * theta_bar * theta_bar  ;
};
/********************************************************************************************/

template <typename particle>
inline particle angular_h_spring_field<particle>::force( particle *part , double theta0 ) {
    /***********   ANGULAR HARMONIC POTENTIAL     **********/               // forces are in units of EPSILON/SIGMA
  f.position *= 0 ;
  cout << "*** ERROR: ANGULAR-HARMONIC-SPRING potential generates no force, but a torque!" << endl;
  exit( EXIT_FAILURE ) ;
...
template <typename particle>
double angular_h_spring_field<particle>::return_pair_potential( double distance ) {
  static double theta_bar ;
  theta_bar = distance ;
  if( theta_bar > HALF_PERIOD ) theta_bar = theta_bar - floor( (theta_bar/HALF_PERIOD+1)/2 ) * PERIOD ;
  else if( theta_bar < -HALF_PERIOD ) theta_bar = theta_bar - ceil( (theta_bar/HALF_PERIOD-1)/2 ) * PERIOD ;
...
template <typename particle>
double angular_h_spring_field<particle>::return_pair_potential( particle *part1 , particle *part2 ) {
...
template <typename particle>
particle angular_h_spring_field<particle>::return_pair_force( particle *i_part , particle *j_part ) {
  // This is an external field, it doesn't generate any pair contribution, hence gives 0
  f.position *= 0.0 ;
  return f ;
};
...
template <typename particle>
double angular_h_spring_field<particle>::compute_energy_contribution( void ) {
  double energy = 0 ;
...
template <typename particle>
inline double angular_h_spring_field<particle>::compute_energy_contribution( particle *part ) {
...
template <typename particle>
double angular_h_spring_field<particle>::delta_energy_constrained_CoM( particle *disp ) {
...
template <typename particle>
double angular_h_spring_field<particle>::lambda_derivative_perpart( void ) {
  cout << "*** ERROR : ANGULAR-HARMONIC-SPRING potential can only be used with the particle class patchy_2D !!" << endl ;
  exit(EXIT_FAILURE) ;
}

template <>
double angular_h_spring_field<patchy_2D>::lambda_derivative_perpart( void ) {
  // This function computes the PER PARTICLE derivative of the external field potential as a function of lambda,
  double der = 0 , theta_bar ;

  int particles_in_list = verlet_list->god->numberOfPart ;
  patchy_2D **particles = verlet_list->god->particles ;
  for( int i=0 ; i<particles_in_list ; i++ ) {
    theta_bar = particles[i]->rotation - equilibrium_positions[i] ;
    if( theta_bar > HALF_PERIOD ) theta_bar = theta_bar - floor( (theta_bar/HALF_PERIOD+1)/2 ) * PERIOD ;
    else if( theta_bar < -HALF_PERIOD ) theta_bar = theta_bar - ceil( (theta_bar/HALF_PERIOD-1)/2 ) * PERIOD ;
    theta_bar /= HALF_PERIOD ;
    der += ( theta_bar * theta_bar ) ;
  }
...
template <typename particle>
void angular_h_spring_field<particle>::compute_forces_contribution(void) {
  /******* ANGULAR-HARMONIC-SPRING potential generates no force, but a torque  ****/
  particle upart ;
};
/********************************************************************************************/

template <typename particle>
double angular_h_spring_field<particle>::compute_virial_contribution(void) {
  /******* ANGULAR-HARMONIC-SPRING potential generates no force, but a torque  ****/
  return 0.0 ;
};
/********************************************************************************************/
...
template <typename particle>
string angular_h_spring_field<particle>::ostr( void ) {
  ostringstream st ;
  st << "ANGULAR HARMONIC SPRING FIELD (spring constant, prefactor, number of minima) : ( LAMBDA=" << lambda << " ; ALPHA_THETA=" << alpha_theta << " ; N_MINIMA=" << n_minima << " )" << endl ;
  return st.str() ;
};
/********************************************************************************************/

template <typename particle>
angular_h_spring_field<particle> *angular_h_spring_field<particle>::copy( void ) {
  angular_h_spring_field *copy = new angular_h_spring_field( lambda , alpha_theta , n_minima ) ;
  return copy ;
};
/********************************************************************************************/

template <typename particle>
void angular_h_spring_field<particle>::compute_per_part_energy_contribution( double *envec ) {
  int particles_in_list = verlet_list->god->numberOfPart ;
  particle **particles = verlet_list->god->particles ;
...
template <typename particle>
void angular_h_spring_field<particle>::compute_per_part_virial_contribution( double *virvec ) {
  /******* ANGULAR-HARMONIC-SPRING potential generates no force, but a torque  ****/
  particle upart ;
};
/********************************************************************************************/
...
template <typename particle>
void angular_h_spring_field<particle>::build_bonds_network( double bonds_cutoff ) {
  cout << "*** WARNING : no method defined to build bonds_network" << endl ;
};
/********************************************************************************************/

template <typename particle>
double * angular_h_spring_field<particle>::dump_parameters( void ) {
