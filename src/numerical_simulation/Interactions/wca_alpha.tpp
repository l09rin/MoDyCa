...
class wca_alpha : public interaction<particle> {     // MD  // energy in units of EPSILON
...
template <typename particle>
wca_alpha<particle>::wca_alpha( double alpha ) {
...
template <typename particle>
void wca_alpha<particle>::generate_by_record( record *file_rec , const char *simulation_kind , configuration<particle> *conf_ptr , int parts_number ) {
	              // CUTOFF
...
template <typename particle>
inline double wca_alpha<particle>::pair_potential( particle *i_part , particle *j_part , double distance ) {  // energy in units of EPSILON
    /***********   WCA + SOLVOPHOBIC POTENTIAL    **********/
  if( distance < 1.122462048309372981433533049679179516232411110613986753440 ) {
    r2_inv = 1.0 / ( distance*distance ) ;
    r6_inv = r2_inv * r2_inv * r2_inv ;
    return 4.0 * ( r6_inv * r6_inv - r6_inv ) - solvophobic_alpha ;
  }else{
...
template <typename particle>
inline particle wca_alpha<particle>::pair_force( particle *i_part , particle *j_part , double distance ) {
    /***********   WCA + SOLVOPHOBIC FORCE    **********/
  if( distance < 1.122462048309372981433533049679179516232411110613986753440 ) {
    r2_inv = 1.0 / ( distance*distance ) ;
    r6_inv = r2_inv * r2_inv * r2_inv ;
...
template <typename particle>
double wca_alpha<particle>::return_pair_potential( double distance ) {
  return pair_potential( NULL , NULL , distance ) + SHIFT ;
};
/********************************************************************************************/

template <typename particle>
double wca_alpha<particle>::return_pair_potential( particle *part1 , particle *part2 ) {
  double energy = 0 ;
  double dist = 0 ;
...
template <typename particle>
particle wca_alpha<particle>::return_pair_force( particle *i_part , particle *j_part ) {
...
template <typename particle>
double wca_alpha<particle>::compute_energy_contribution( void ) {
  double energy = 0 ;
  double dist = 0 ;
...
template <typename particle>
inline double wca_alpha<particle>::compute_energy_contribution( particle *part ) {
...
template <typename particle>
double wca_alpha<particle>::delta_energy_constrained_CoM( particle *disp ) {
...
template <typename particle>
double wca_alpha<particle>::lambda_derivative_perpart( void ) {
...
template <typename particle>
void wca_alpha<particle>::compute_forces_contribution(void) {
  double dist = 0 ;
  neighbour_list<particle> *walker = NULL ;
...
template <typename particle>
double wca_alpha<particle>::compute_virial_contribution(void) {
  double dist = 0 ;
  neighbour_list<particle> *walker = NULL ;
...
template <typename particle>
string wca_alpha<particle>::ostr( void ) {
  ostringstream st ;
...
template <typename particle>
wca_alpha<particle> *wca_alpha<particle>::copy( void ) {
  wca_alpha *copy = new wca_alpha( solvophobic_alpha ) ;
  return copy ;
};
/********************************************************************************************/

template <typename particle>
void wca_alpha<particle>::compute_per_part_energy_contribution( double *envec ) {
  double energy = 0 ;
  double dist = 0 ;
  neighbour_list<particle> *walker = NULL ;
  particle *apparent_near = NULL ;
...
template <typename particle>
void wca_alpha<particle>::compute_per_part_virial_contribution( double *virvec ) {
  double dist = 0 ;
  neighbour_list<particle> *walker = NULL ;
  particle *apparent_near = NULL , deltaF ;
  apparent_near = new particle ;
...
template <typename particle>
void wca_alpha<particle>::build_bonds_network( double bonds_cutoff ) {
  cout << "*** WARNING : no method defined to build bonds_network" << endl ;
};
/********************************************************************************************/

template <typename particle>
double * wca_alpha<particle>::dump_parameters( void ) {
